...
public:
    ...
    // noise
    double gyro_bias_sigma = 0.00005;     //  陀螺仪零偏随机游走
    double acc_bias_sigma = 0.0005;	  //  加速度计零偏随机游走

    //double gyro_bias_sigma = 1.0e-5;
    //double acc_bias_sigma = 0.0001;

    double gyro_noise_sigma = 0.015;    // rad/s * 1/sqrt(hz)      陀螺仪高斯白噪声
    double acc_noise_sigma = 0.019;      //　m/(s^2) * 1/sqrt(hz)  加速度计高斯白噪声
